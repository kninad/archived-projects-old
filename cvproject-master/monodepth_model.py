...
class MonodepthModel(object):
    ...
    #~ def get_l2_norm(self, dep):
        #~ return tf.norm(dep, axis=(1,2))
    ...
    def get_disparity_smoothness(self, disp, pyramid):
        ...
        eps = 1e-7
        disp_e = [d + eps for d in disp]
        depths = [1. / d for d in disp_e]
        image_gradients_x = [self.gradient_x(dep)/tf.norm(dep, axis=(1,2)) for dep in depths]
        image_gradients_y = [self.gradient_y(dep)/tf.norm(dep, axis=(1,2)) for dep in depths]
        ...
        smoothness_y = [disp_gradients_x[i] * weights_x[i] + disp_gradients_y[i] * weights_y[i] for i in range(4)]
        return smoothness
